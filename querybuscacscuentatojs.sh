...
')
	...
	if [ -z "$encuentra" ];then
		...
		if [ 0$len -gt 0 ];then
			...
			if [ -z "$errores" ];then
			    asset=""
			    encuentra=$(cat "$listf"|$PrPWD/stdbuscaarg " asset[")
			    if [ -n "$encuentra" ];then
				asset=$(cat "$listf"|$PrPWD/stdcdr "asset[" |$PrPWD/stdcdr "=" | $PrPWD/stdcarsin ";")
			    fi
			    if [ -z "$encuentra" ];then
				encuentra=$(cat "$listf"|$PrPWD/stdbuscaarg " cuenta[")
				if [ -n "$encuentra" ];then
				    asset="\"DATOS_CUENTA_\""
				fi
			    fi
			    if [ -n "$asset" ];then
				echo " . . . . . .       $listf       $asset"
				encuentra=$(cat "$nomprogvars"|$PrPWD/stdbuscaarg "var asset=new Array();")
				if [ -z "$encuentra" ];then
				    echo "var asset=new Array();" >> "$nomprogvars"
				fi
				codigo="asset[asset.length]=$asset;"
				encuentra=$(cat "$nomprogvars"|$PrPWD/stdbuscaarg "$codigo")
				if [ -z "$encuentra" ];then
				    echo "$codigo" >> "$nomprogvars"
				fi
				cat $PrPWD/stddeclaracionesdevariable.c | $PrPWD/stdcar "char prefix" > "$nomprogvars-variables.c"
				echo "[7]={'a','s','s','e','t','s','_'};" >> "$nomprogvars-variables.c"
				cat $PrPWD/stddeclaracionesdevariable.c | $PrPWD/stdcdr "char prefix" | $PrPWD/stdcdr ";" >> "$nomprogvars-variables.c"
				gcc -o "$nomprogvars-variables" "$nomprogvars-variables.c"
				variables=$(cat "$fn" |"$nomprogvars-variables"|$PrPWD/stddeclaracionesdevariable_tojs|tr -d '
')
				encuentra="ALGO"
				while [ -n "$encuentra" ];do
					...
					if [ -z "$encuentra" ];then
						...
						if [ -z "$encuentra" ];then
							...
							codigo="var contador=0;while(asset[contador]!=$asset && contador<asset.length){contador++;};if(contador<asset.length) $rev[contador]=$reu;"
							encuentra=$(cat "$nomprogcods"|$PrPWD/stdbuscaarg "$codigo")
							if [ -z "$encuentra" ];then
							    codigo2="var contador=0;while(asset[contador]!=$asset && contador<asset.length){contador++;};if(contador<asset.length) $rev[contador]="
							    if [ -n "$encuentra" ];then
