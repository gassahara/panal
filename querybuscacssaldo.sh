...
' ';')
		    ...
		    varis=$(echo -n ";$variables" |$PrPWD/stdbuscaarg "double prefix_saldo")
		    if [ -n "$varis" ];then
			varos="$varos$varis"
			echo "$varos"
		    ...
		    varis=$(echo -n ";$variables"|$PrPWD/stdbuscaarg "char prefix_MAC[")
		    if [ -n "$varis" ];then
			varos="$varos$varis"
			echo "$varos"
		    else
			...
			saldo=$(echo -n ";$variables"|$PrPWD/stdcdr "double prefix_saldo"|$PrPWD/stdcdr "="|$PrPWD/stdcarsin ";")
			if [ 0$saldo -lt 5 ]; then
			    exit 0
			fi
			cedula=$(echo -n ";$variables"|$PrPWD/stdcdr "char prefix_cedula["|$PrPWD/stdcdr "]=\""|$PrPWD/stdcarsin "\"")
			MAC=$(echo -n ";$variables"|$PrPWD/stdcdr "char prefix_MAC["|$PrPWD/stdcdr "]="|$PrPWD/stdcarsin ";"|tr -d ':')
			...
			if [ -f "$PrPWD/users/cuentas/$cedula-$MAC" ];then
			    utcc=$(dd if=/dev/random bs=1 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
			    while [ -f "$dirfn/$utcc.c" ];do
				utcc=$(dd if=/dev/random bs=1 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
			    done
			    saldo_c=cat $PrPWD/users/clientes/$cedula | $PrPWD/stdcdr "double saldo="| $PrPWD/stdcarsin ";"
			    echo "s $saldo"
			    echo "c 0$saldo_c"
			    echo "0$saldo_c + 0$saldo" | bc -l
			    saldo=$(echo "0$saldo_c + 0$saldo" | bc -l)
			    cat $PrPWD/users/clientes/$cedula | $PrPWD/stdcar "double saldo=" > $utcc
			    echo  "0$saldo;" >> $utcc
			    cat $PrPWD/users/clientes/$cedula | $PrPWD/stdcdr "double saldo=" | $PrPWD/stdcdr ";" >> $utcc
			    cp -v $utcc "$PrPWD/users/clientes/$cedula"
			    mkdir $PrPWD/users/output
			    echo "$saldo ABONADOS  CON EXITO A LA CUENTA" > $PrPWD/users/output/$userd/$respuesta
			else
