...
if [ 0$pasa -eq 0 ];then
	...
	if [ -z "$shaf" ];then
	    ...
	    campof=$(cat $fn.post |$PrPWD/stdhttpcontent |$PrPWD/stdcdr "boundary=" | $PrPWD/stdtohex | $PrPWD/stdcarsin 0A )
	    campog=$(echo -n "$campof" | $PrPWD/stdfromhex)
	    dashes=$(echo -n "---" | $PrPWD/stdtohex )
	    name1=$(echo -n 'name="' | $PrPWD/stdtohex )
	    i=1
	    n=0
	    cad1=$(cat $fn.post | $PrPWD/stdtohex)
		...
		i=$((i+1))
 	    ...
 	    if [ 0$r -gt 3 -a "$previs" != "si" "$hacer" = "si" ];then
