...
' ';')
			...
			if [ -f "$PrPWD/users/cuentas/$cedula-$MAC" ];then
			    ...
			    mkdir $PrPWD/users/output/$userd
			    echo "CUENTA YA EXISTE" > $PrPWD/users/output/$userd/$respuesta
			else
			    mkdir $PrPWD/users/clientes
			    cp -v "$listf" $PrPWD/users/cuentas/$cedula-$MAC
			    mkdir $PrPWD/users/output
			    mkdir $PrPWD/users/output/$userd
			    echo "Cuenta Agregado con Exito" > $PrPWD/users/output/$userd/$respuesta
			fi
		    fi
		fi
		mkdir peticiones
