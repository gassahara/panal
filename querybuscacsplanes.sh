...
' ';')
		    echo "$variables"
		    varos="";
		    varis=$(echo -n ";$variables" |$PrPWD/stdbuscaarg "double prefix_subida")
		    if [ -n "$varis" ];then
			varos="$varos$varis"
			echo "$varos"
		    ...
		    varis=$(echo -n ";$variables"|$PrPWD/stdbuscaarg "double prefix_descarga")
		    if [ -n "$varis" ];then
			varos="$varos$varis"
			echo "$varos"
		    else
		    ...
		    varis=$(echo -n ";$variables"|$PrPWD/stdbuscaarg "double prefix_monto")
		    if [ -n "$varis" ];then
			varos="$varos$varis"
			echo "$varos"
		    else
		    ...
		    varis=$(echo -n ";$variables"|$PrPWD/stdbuscaarg "char prefix_plan[")
		    if [ -n "$varis" ];then
			varos="$varos$varis"
			echo "$varos"
		    else
		    ...
		    if [ "$varos" = "****" ];then
			subida=$(echo -n ";$variables"|$PrPWD/stdcdr "double prefix_subida"|$PrPWD/stdcdr "="|$PrPWD/stdcarsin ";")
		        descarga=$(echo -n ";$variables"|$PrPWD/stdcdr "double prefix_descarga"|$PrPWD/stdcdr "="|$PrPWD/stdcarsin ";")
			plan=$(echo -n ";$variables"|$PrPWD/stdcdr "char prefix_plan["|$PrPWD/stdcdr "]=\""|$PrPWD/stdcarsin "\"")
			monto=$(echo -n ";$variables"|$PrPWD/stdcdr "char prefix_monto"|$PrPWD/stdcdr "="||$PrPWD/stdcarsin ";")
			respuesta=$(echo -n ";$variables"|$PrPWD/stdcdr "char prefix_respuesta["|$PrPWD/stdcdr "]=\""|$PrPWD/stdcarsin "\"")
			echo "$fechan $plan $fecha $cedula .............."
			mkdir $PrPWD/users/planes
			if [ -f "$PrPWD/users/planes/$plan" ];then
			    mkdir $PrPWD/users/tooutput
			    echo "PLAN YA EXISTE CON ESE NOMBRE" > $PrPWD/users/tooutput/$userd/$respuesta
			else
			    cp -v "$listf" $PrPWD/users/planes/$plan
			    utcc=$(dd if=/dev/random bs=1 count=5 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
			    echo "<option value=\"$utcc\"> $plan </option>" >> $PrPWD/users/planes.html
			    echo "PLAN REGISTRADO CON EXITO..." > $PrPWD/users/tooutput/$userd/$respuesta
			fi
		    fi
		fi
		mkdir peticiones
