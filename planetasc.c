...
int main(int argc, char *argv[] ) {
  ...
  double DSA=90+lsAD;
  double NSA=180-DSA;
  printf("DECL: %06f MD: %06f AD: %06f DSA: %06f NSA: %06f\n", DECL, MD, lsAD, DSA,  NSA);
  double dif=0;
  if(ARMC>RA)  dif=MD-DSA;
  else dif=MD-NSA;
  printf("dif: %06f \n", dif );
  double pole=dif+(tan(obslat*DEGTORAD)*RADTODEG);
  printf("pole: %06f \n", pole );
  DSA=pole+DECL;
  printf("AD: %06f \n", DSA );
  printf("OA: %06f %06f \n", RA-DSA, fmod(RA-DSA, 360) );
  ...
  if(obslat<0 && DECL>0) lsAD=-lsAD;
  */
  if(AO==1 && obslat<0) RA+=lsAD;
  if(AO==0 && obslat<0) RA-=lsAD;
  if(AO==0 && obslat>0) RA+=lsAD;
  if(AO==1 && obslat>0) RA-=lsAD;
  printf("RA(Asc): %06f \n", RA);
