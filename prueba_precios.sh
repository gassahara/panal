...
')
	...
	if [ "$symbol" = "LTCETH" ];then pricec=$(echo "$price + ($price*0.0025)"|bc -l) ; fi
	if [ "$symbol" = "BNBETH" ];then pricec=$(echo "$price + ($price*0.0025)"|bc -l) ; fi
	if [ "$symbol" = "ADAETH" ];then pricec=$(echo "$price + ($price*0.0025)"|bc -l) ; fi
	if [ "$symbol" = "SOLETH" ];then pricec=$(echo "$price + ($price*0.0021)"|bc -l) ; fi
	if [ "$symbol" = "ETCETH" ];then pricec=$(echo "$price + ($price*0.0021)"|bc -l) ; fi
 	echo $pricec . $pricet
	price=$pricec
...
'); fi
    ...
    precios=$(curl -L 'https://api.binance.com/api/v3/ticker/price?symbols=%5B%22ETCETH%22,%22SOLETH%22,%22XRPETH%22,%22NEOETH%22,%22ADAETH%22,%22MATICETH%22,%22EOSETH%22,%22BNBETH%22,%22LTCETH%22,%22DOTETH%22%5D' 2>/dev/null | sed "s/,/$/g" | sed "s/:/=/g" | sed 's/"symbol"/symbol/g' | sed 's/"price"=/price=/g' | tr [{}$] '
')
...
'|wc -c | $PrPWD/stdcarsin " ")
		if [ $(expr 0 + $len) -gt 2 ];then
		    echo "SSS $sumab"
		    if [ $(expr 0 + $sumab) -gt 2 ];then
			eval "\$M$symbol=\" \""
			linea="";
		    fi
		fi
...
#		echo "+ $mas - $menos = $iguales"
		...
		if [ 0$len -eq 5 ];then
		    ...
		    saldo=$saldom
		    echo "scale=$long;a=($pricea-($pricea*0.001));a/1"|bc -l
			...
			if [ "$symbol" = "SOLETH" ];then long=6;fi
			...
			if [ "$symbol" = "SOLETH" ];then longs=4;fi
			if [ "$symbol" = "MATICETH" ];then longs=1;fi
