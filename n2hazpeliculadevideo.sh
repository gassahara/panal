...
if [ 0$pasa -eq 0 ];then
	...
	if [ -n "$enproceso" -a $(echo "$enproceso" | $PrPWD/stdbuscaarg_count "0A" ) -lt 1 ];then
		...
		echo "_____ $P2PWD $namd"
		$P2PWD/listadodirectorio_files_extension 1.jpg #|$P2PWD/stdcdr "$PrPWD"|$P2PWD/stdtohex
		echo "_____"
		imag=""
		if [ -f "poster.jpg" ];then
		    imag="$namd/poster.jpg"
		else
		    if [ -z "$imag" ];then
			imag=$($P2PWD/listadodirectorio_files_extension 2.jpg|$P2PWD/stdcdr "$PrPWD"|$P2PWD/stdtohex|$P2PWD/stdcarsin 0A|$P2PWD/stdfromhex)
			if [ -z "$imag" ];then
			    imag=$($P2PWD/listadodirectorio_files_extension .jpg|$P2PWD/stdtohex|$P2PWD/stdcarsin 0A|$P2PWD/stdfromhex)
			fi
		    fi
		fi
		echo "[ $imag ]                    $namo <<<<<<<<"
		cd "$PaPWD"
		lista4=$(echo -n "<p style='width:30%;height:22%;float:left;' id='$id1' onclick='$listac;uvideo();'><img src='$imag' style='height:120px;'><br>$namo</p>" | $PrPWD/stdtohex )
    ...
    if [ -n "$lista2" ];then
	listav=" Videos <hr> <div id='directoriov' style='width:100%;'></div> <div style='width:90%;left:5%;position:relative;background-color:green;height:110%;display:flex;flex-wrap:wrap;overflow:scroll;' name='videos' id='videos' style='width:100%;' >$lista2 </div>"
    fi
    ...
    cp -v $PrPWD/uD2.html $PrPWD/uD3.html
    #$0 &
    disown
fi
