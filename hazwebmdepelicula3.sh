...
while [ -z "$archivos" ];do
  IFS=$'\n'
  touch -d $fechaa $0
  for d in $(find uploads -type f -newermm $0 -exec stat -c "%s@%n@%z" "{}" \; | sort -u ) ; do
	...
	if [ "$fecha" -ge "$fechaa" ];then
		...
		#echo "<<< $irchivo :: $d >>>"
   	        ffprocs=$(top -b -n 1 | grep convierteawebm | grep -v grep | wc -l)
...
echo "ffprocs $ffprocs"
		if [ $ffprocs -lt 4 ];then
			...
			#echo %%%% $archivo
			if [ ! -z "$archivo" ];then
			    setsid ./convierteawebm.sh "$irchivo" &
			    sleep 3
			    echo -e "$ffprocs procesos $irchivo -> webm"
			fi
		    fi
		fi
