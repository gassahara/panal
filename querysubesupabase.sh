...
# Retrieve the configured Supabase endpoint
...
')
    ...
    encuentra=$(cat $nomprograma.memoria 2>/dev/null | $PrPWD/stdbuscaarg ";$listf;$chacha;")
    if [ -z "$encuentra" ];then
...
' >> $outputdelc-2.c
	    ...
	    echo "Uploading to Supabase: $remotepath"
	    # Instead of uppy.php on 000webhostapp, we POST to Supabase billboard-storage function.
	    # It expects fileToUpload and namo to behave like uppy.php.
	    curl -L --retry-all-errors --retry 30 -X POST -F "fileToUpload=@$outputdelc.js" -F "namo=$fn2.js" -F "submit=submit" "$remotepath"
	fi
    fi
done
$0&
