...
if [ -n "$nn" ];then
    ...
    if [ -n "$esta" ];then
	echo "ESTA"
	query=$(cat $nn | ./stdcdr "<td>query<" | ./stdcdr "/td><td" | ./stdcar "
")
#	query=$(cat $nn | grep "<td>query<" | sed "s/<td\>/;/g; s/<\/td\>/;/g " | tr "[\n\r\t]" ";" | cut -d ";" -f5)
	...
	while read e;do
		...
		echo "</pre></body></html>"
		if [ ! -f $nombre-palabras3.html ];then
		while read ee;do
		    echo "<a href=\"$ee\"> $ee </a>" >> $nombre-palabras3.html
		done < <(ls -1 palabras3/ )
		fi
		echo "<td><a href=\"$nombre_$e.html\"> Ver Codigo </a> </td></tr><tr><td> <a href=\"$nombre-palabras3.html\"> Ver Catalogo </a> </td> </tr>  <tr><td><a onclick=\"document.getElementById('ifra').src ='funciones/$query$fn.html'\"> Ver Resultado de $e con $query </a> </td></tr></table> " >> $nombre.html
		ia=$((ia+1))
	    fi
	done < <(ls -1 funciones/ )
