...
' | sha512sum | $PrPWD/stdcarsin ' ')
			    ...
			    echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   $respuesta   ($encuentrac) ($namepublic)"
		            ...
		            namesign=$(echo -n ";$variables"|$PrPWD/stdcdr "char prefix_nameofSignatary["|$PrPWD/stdcdr "="|$PrPWD/stdcarsin ";"|tr -d '"' | sed 's/%\([0-9A-Fa-f][0-9A-Fa-f]\)/\\x\1/g' | xargs -0 echo -e)
			    namesignRemote=$(echo "$namesign" | tr -d '
' | sha512sum | $PrPWD/stdcarsin ' ')
			    namesignRemote=$(echo "$namesignRemote .js"|tr -d ' ')
			    namesignpublic=$(echo "$namesignRemote public"| tr -d ' ' | sha512sum | $PrPWD/stdcarsin ' ')
			    echo "NAMESIGN    $namesignRemote ($namesign)"
			    echo "PUBLIC  $namesignpublic"
			    respuestab=$(curl -L "$remotepath/fretfile.php?fname=$namesignpublic.js" 2>/dev/null | $PrPWD/stdcdr 'error' | $PrPWD/stdcdr '"' | $PrPWD/stdcarsin '"'  | $PrPWD/stdbuscaarg "Success")
			    if [ -z "$respuestab" ];then
				echo "$namesign not found"
				exit
			    fi
			    respuesta=$(echo "$respuestaa $respuestab")
			    echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   $respuesta   ($encuentrac) ($namesignpublic)"
				...
				dirSignatary="$PrPWD/users/input/$namesign"
				dirSignataryTokens="$PrPWD/users/input/$namesign/tokens"
				mkdir $dirSignatary
				...
				while [ -n "$busca" ];do
					...
					mv "$dirDestinataryTokens/$fname" $dirTokensDeleted
				    fi
				done

				if [ "$ammountTotal" -ge "$ammount" ];then
				    resto=$(expr 0$ammountTotal - 0$ammount)
				    echo "< $resto >" "$ammountTotal" -ge "$ammount"
...
#				    cat $PaPWD/$billscc.c
				    errores=$(gcc -o "$PaPWD/$billscc" "$PaPWD/$billscc.c" 2>&1)
...
'  >> "$PaPWD/$textcc"
					...
					OTP_resource=$(echo "$variables" | $PrPWD/stdcdr "OTP_resource=" | $PrPWD/stdcarsin ";")
					OTP=$(echo "$variables" | $PrPWD/stdcdr " OTP=" | $PrPWD/stdcarsin ";")
					respuestab=$(curl -L "$remotepath/fretfile.php?fname=$namesignpublic.js" 2>/dev/null )
					encuentra=$(echo " $respuestab" |$PrPWD/stdbuscaarg 'Success')
					if [ -n "$encuentra" ] ; then
					    echo "SUBIR"
					    boundaryR=$(dd if=/dev/urandom bs=1 skip=20 count=20 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
					    ...
					    echo "Content-Type: multipart/mixed; boundary=$boundary" > "$PaPWD/$output"
					    curl -L "$remotepath/fretfile.php?fname=$namesignpublic.js" 2>/dev/null | $PrPWD/stdcdr "content=" | $PrPWD/stdcarsin ";" | tr -d '"' | base64 -d | $PrPWD/stdcdr '`' | $PrPWD/stdcarsin '`' > $utcc.public
					    signedoutput=$(cat $PaPWD/$textcc| gpg  --homedir $PrPWD/user/ --no-default-keyring --keyring $PrPWD/user/key.key --trustdb-name $PrPWD/user/trustdb.gpg --armor  --encrypt -f  $utcc.public - | gpg  --homedir $PrPWD/user/ --no-default-keyring --keyring $PrPWD/user/key.key --trustdb-name $PrPWD/user/trustdb.gpg --clearsign )				    
					    echo "$datefield" >> "$PaPWD/$output"
					    echo "Subject: #sendcoin" >> "$PaPWD/$output"
					    echo '
...
')
					    rm -v "$textcc" "$textcc.c" "$temptextcc" "$temptextcc.c" "$utcc.public"
					    namel=$(echo "$namesignRemote"|tr -d '
'|wc -c)
					    encryptedoutputl=$(echo "$encryptedoutput"| tr -d '
'|wc -c)
					    texto=$(echo " $($PrPWD/aleatorio|$PrPWD/stdcdrn 2) int main() {  $($PrPWD/aleatorio) char nameofindex[$namel]=\"$namesignRemote\"; $($PrPWD/aleatorio)  char command[6]=\"APPEND\"; $($PrPWD/aleatorio)  char content[$encryptedoutputl]=\"$encryptedoutput\"; $($PrPWD/aleatorio)  }")
					    encryptedoutput=$(echo "$texto"| gpg  --homedir $PrPWD/user/ --no-default-keyring --keyring $PrPWD/user/key.key --trustdb-name $PrPWD/user/trustdb.gpg --armor  --encrypt -f $serverPublic)
					    encriptedOutputFiles=$(dd if=/dev/random bs=1 skip=20 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
					    while [ -f "$PaPWD/$encriptedOutputFiles" ];do
						encriptedOutputFiles=$(dd if=/dev/random bs=1 skip=20 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
