...
' ';')
			...
			echo "varos passed;"
		        ...
		        contenttopublish=$(echo -n ";$variables"|$PrPWD/stdcdr "char prefix_content["|$PrPWD/stdcdr "="|$PrPWD/stdcarsin ";"|tr -d '"')
			echo "COMMAND $command";
		        ammount=$(echo -n ";$variables"|$PrPWD/stdcdr "long prefix_ammount="|$PrPWD/stdcarsin ";")
			echo "AMMOUNT $ammount";
		        nameDestinatary=$(echo -n ";$variables"|$PrPWD/stdcdr "char prefix_nameofSignatary["|$PrPWD/stdcdr "="|$PrPWD/stdcarsin ";"|tr -d '"' | sed 's/%\([0-9A-Fa-f][0-9A-Fa-f]\)/\\x\1/g' | xargs -0 echo -e)
		        nameSignatary=$(echo -n ";$variables"|$PrPWD/stdcdr "char prefix_nameofDestinatary["|$PrPWD/stdcdr "="|$PrPWD/stdcarsin ";"|tr -d '"' | sed 's/%\([0-9A-Fa-f][0-9A-Fa-f]\)/\\x\1/g' | xargs -0 echo -e)
			nameSignataryRemote=$(echo "$nameDestinatary" | tr -d '
' | sha512sum | $PrPWD/stdcarsin ' ')
			...
			echo "NAMEDestinatary    $nameSignataryRemote"
			echo "PUBLICDestinatary  $namepublicSignataryRemote"
			publicKeyofSignatary=$(curl -L "$remotepath/fretfile.php?fname=$namepublicSignataryRemote.js" 2>/dev/null | $PrPWD/stdcdr "content=" | $PrPWD/stdcdr '"'  | $PrPWD/stdcarsin '"' | base64 -d | $PrPWD/stdcdr '`' | $PrPWD/stdcarsin '`')
			publicKeyofSignataryResult=$(echo "$publicKeyofDestinatary" | wc -c)
			nameDestinataryRemote=$(echo "$nameSignatary" | tr -d '
' | sha512sum | $PrPWD/stdcarsin ' ')
			nameDestinataryRemote=$(echo "$nameDestinataryRemote .js"|tr -d ' ')
			namepublicDestinataryRemote=$(echo "$nameDestinataryRemote public"| tr -d ' ' | sha512sum | $PrPWD/stdcarsin ' ')
			echo "NAMESignatary    $nameDestinataryRemote"
			echo "PUBLICSignatary  $namepublicDestinataryRemote"
			publicKeyofDestinatary=$(curl -L "$remotepath/fretfile.php?fname=$namepublicDestinataryRemote.js" 2>/dev/null | $PrPWD/stdcdr "content=" | $PrPWD/stdcdr '"'  | $PrPWD/stdcarsin '"' | base64 -d  | $PrPWD/stdcdr '`' | $PrPWD/stdcarsin '`')
			publicKeyofDestinataryResult=$(echo "$publicKeyofSignatary" | wc -c)
			...
			if [ -n "$encuentrac" ] ; then
			    dirSignatary="$PrPWD/users/input/$nameDestinatary"
			    dirSignataryTokens="$PrPWD/users/input/$nameDestinatary/tokens"
			    mkdir $dirSignatary
			    mkdir $dirSignataryTokens
			    echo $dirSignatary $dirSignataryTokens
			    dirDestinatary="$PrPWD/users/input/$nameSignatary"
			    dirDestinataryTokens="$PrPWD/users/input/$nameSignatary/tokens"
			    filename=$(echo -n ";$variables"|sha512sum | $PrPWD/stdcarsin ' ' )
			    dirDestinataryTokensFN="$dirDestinataryTokens/$filename"
			    mkdir $dirDestinatary
			    mkdir $dirDestinataryTokens
			    echo $dirDestinatary $dirDestinataryTokens
			    
			    if [ ! -d  "$dirDestinataryTokensFN" ];then
				echo "$filename" > $temptextcc
				...
				echo "Subject: #PPVpermission" > "$PaPWD/$textcc"
...
'  >> "$PaPWD/$textcc"
				...
				respuestab=$(curl -L "$remotepath/fretfile.php?fname=$namepublicDestinataryRemote.js" 2>/dev/null)
				...
				if [ -n "$encuentra" ] ; then
				    ...
				    curl -L "$remotepath/fretfile.php?fname=$namepublicDestinataryRemote.js" 2>/dev/null | $PrPWD/stdcdr "content=" | $PrPWD/stdcarsin ";" | tr -d '"' | base64 -d | $PrPWD/stdcdr '`' | $PrPWD/stdcarsin '`' > $utcc.public
...
'|wc -c)
				    texto=$(echo " $($PrPWD/aleatorio|$PrPWD/stdcdrn 2) int main() {  $($PrPWD/aleatorio) char nameofindex[$namel]=\"$nameDestinataryRemote\"; $($PrPWD/aleatorio)  char command[6]=\"APPEND\"; $($PrPWD/aleatorio)  char content[$encryptedoutputl]=\"$encryptedoutput\"; $($PrPWD/aleatorio)  }")
			    ...
			    while [ -f "$eyedirectory/unencrypted/$billscc.c" ];do
				billscc=$(dd if=/dev/urandom bs=1 skip=20 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
			    done
			    sleep 33
			    mv -v $mouthdirectory/$fn2 $eyedirectory/unencrypted/$billscc.c
			    echo $dirfn/b$fn2
			    exit
			else				
			    listaTokens=$(echo "$dirDestinataryTokensFN" | $PrPWD/listadodirectorio_files_from_std_extension_c|tr '
' ' ')
			    tokensAmmount=0
			    ammountRes=0
...
'|$PrPWD/stdbuscaarg " ")
			    ...
			    echo "A $tokensAmmount ($dirDestinataryTokensFN) ::"			    
...
'  >> "$PaPWD/$billscc-ii.c" 
				...
				rm -v "$PaPWD/$billscc" "$PaPWD/$billscc.c"
				if [ -z "$errors" ];then
				    cp $dirSignataryTokens/* $dirTokens
				    textcc=$(dd if=/dev/urandom bs=1 skip=20 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
				    while [ -f "$PaPWD/$textcc" ];do
					textcc=$(dd if=/dev/urandom bs=1 skip=20 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
				    done
...
'  >> "$PaPWD/$textcc"
				    ...
				    OTP_resource=$(echo "$variables" | $PrPWD/stdcdr "OTP_resource=" | $PrPWD/stdcarsin ";")
				    OTP=$(echo "$variables" | $PrPWD/stdcdr " OTP=" | $PrPWD/stdcarsin ";")
				    respuestab=$(curl -L "$remotepath/fretfile.php?fname=$namepublicSignataryRemote.js" 2>/dev/null)
				    encuentra=$(echo "$respuestab" |$PrPWD/stdbuscaarg 'Success')
				    if [ -n "$encuentra" ] ; then
					echo "SUBIR"
					boundaryR=$(dd if=/dev/urandom bs=1 skip=20 count=20 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
					...
					echo "Content-Type: multipart/mixed; boundary=$boundary" > "$PaPWD/$output"
					curl -L "$remotepath/fretfile.php?fname=$namepublicSignataryRemote.js" 2>/dev/null | $PrPWD/stdcdr "content=" | $PrPWD/stdcarsin ";" | tr -d '"' | base64 -d | $PrPWD/stdcdr '`' | $PrPWD/stdcarsin '`' > $utcc.public
					signedoutput=$(cat $PaPWD/$textcc| gpg  --homedir $PrPWD/user/ --no-default-keyring --keyring $PrPWD/user/key.key --trustdb-name $PrPWD/user/trustdb.gpg --armor  --encrypt -f  $utcc.public - | gpg  --homedir $PrPWD/user/ --no-default-keyring --keyring $PrPWD/user/key.key --trustdb-name $PrPWD/user/trustdb.gpg --clearsign )				    
					echo "$datefield" >> "$PaPWD/$output"
					echo "Subject: #sendcoin" >> "$PaPWD/$output"
					echo '
...
')
					rm -v "$textcc" "$textcc.c" "$temptextcc" "$temptextcc.c" "$utcc.public"
					namel=$(echo "$nameSignataryRemote"|tr -d '
'|wc -c)
					encryptedoutputl=$(echo "$encryptedoutput"| tr -d '
'|wc -c)
					texto=$(echo " $($PrPWD/aleatorio|$PrPWD/stdcdrn 2) int main() {  $($PrPWD/aleatorio) char nameofindex[$namel]=\"$nameSignataryRemote\"; $($PrPWD/aleatorio)  char command[6]=\"APPEND\"; $($PrPWD/aleatorio)  char content[$encryptedoutputl]=\"$encryptedoutput\"; $($PrPWD/aleatorio)  }")
					encryptedoutput=$(echo "$texto"| gpg  --homedir $PrPWD/user/ --no-default-keyring --keyring $PrPWD/user/key.key --trustdb-name $PrPWD/user/trustdb.gpg --armor  --encrypt -f $serverPublic)
					encriptedOutputFiles=$(dd if=/dev/random bs=1 skip=20 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
					while [ -f "$PaPWD/$encriptedOutputFiles" ];do
					    encriptedOutputFiles=$(dd if=/dev/random bs=1 skip=20 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
...
'  >> "$PaPWD/$billscc.c"
				...
				ldestokens=$(echo "$dirDestinataryTokensFN/" | tr -d '
' | wc -c)
				ldestokens=$(expr $ldestokens + 21)
				cat $PaPWD/$billscc.c | $PrPWD/stdcar "char tokensDirplusFilename[" > "$PaPWD/$billscc-ii.c"
				echo "$ldestokens]="'"'"$dirDestinataryTokensFN/"'"'";" | tr -d '
'  >> "$PaPWD/$billscc-ii.c" 
				cat $PaPWD/$billscc.c | $PrPWD/stdcdr "char tokensDirplusFilename[" | $PrPWD/stdcdr ';' >> "$PaPWD/$billscc-ii.c"
				mv -v "$PaPWD/$billscc-ii.c" "$PaPWD/$billscc.c"
				echo "_ _ _ _ _ _ _"
				...
				if [ -z "$errors" ];then
				    rm -v "$PaPWD/$billscc" "$PaPWD/$billscc.c"
				    cp $dirDestinataryTokensFN/* $dirTokens
				    textcc=$(dd if=/dev/urandom bs=1 skip=20 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
				    while [ -f "$PaPWD/$textcc" ];do
					textcc=$(dd if=/dev/urandom bs=1 skip=20 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
				    done
...
'|wc -c)
				...
				OTP_resource=$(echo "$variables" | $PrPWD/stdcdr "OTP_resource=" | $PrPWD/stdcarsin ";")
				OTP=$(echo "$variables" | $PrPWD/stdcdr " OTP=" | $PrPWD/stdcarsin ";")
				contentd=$(echo "$contenttopublish"|base64 -d)
				#echo "..:: $contentd  ::.."
