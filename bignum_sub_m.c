...
void bignum_sub(DTYPE *a, DTYPE * b, int lena, int lenb, DTYPE *c) {
  DTYPE_TMP res;
  DTYPE_TMP tmp1;
  DTYPE_TMP tmp2;
  int borrow = 0;
...
int main(int argc, char *argv[] ) {
  ...
  bignum_sub(Anum, Bnum, lena, lenb, resulta);
  int lens=(1+lenc)*(WORD_SIZE*2);
  char str[lens];
  memset(str, 0, lens);
