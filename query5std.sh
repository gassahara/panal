...
while [ -n $(dd if=$archivo bs=1 skip=$skips count=1) ];do
    ...
    dd if=$archivo bs=1 skip=$skips count=$indicecabecera  2>/dev/null 
    echo "-|-|-|-|-|-|-|-|-|-|-|-|-|-"
    From=""
    contype=""
    b64=""
    ...
    From=$(dd if=$archivo bs=1 skip=$skips count=$indicebacera |  ./stdcdr "From: " | ./stdcarsin $'\n' )
    echo "From>$From"
    ...
    echo "enc=$b64"
    bou=$(dd if=$archivo bs=1 skip=$skips count=$indicecabecera 2>/dev/null |  ./stdcdr "boundary=" )
    echo "Bou=$bou"
    ...
    if [ 0$esta -ne 2 -a -n "$From" -a -z "$campf" -a -z "$size" -a -z "$bou" ];then
	fname=$(dd if=$archivo bs=1 skip=$skips count=$indicecabecera  2>/dev/null | ./stdcdr "filename=\"" | ./stdcarsin "\"")
	if [ -z "$fname" ];then
	    fname=$(dd if=$archivo bs=1 skip=$skips count=$indicecabecera  2>/dev/null | ./stdcdr " name=\"" | ./stdcarsin "\"")
	    if [ -z "$fname" ];then
		fname=$(echo "$archivo2-$skips-CORTO" | ./stdcarsin "." )
	...
	From="From"
	echo "$'$cars'From "
	cadd="dd if=$archivo bs=1 skip=$skips | ./stdbuscaarg $'$cars$From '"
	eval $cadd
	cadd="dd if=$archivo bs=1 skip=$skips | ./stdcarsin $'$cars$From ' > \"$fname\" "
	eval $cadd
	echo "::::::::::::::::::::::::::::::::::::::::"
	echo "::::::::::::::::::::::::::::::::::::::::"
	echo "::::::::::::::::::::::::::::::::::::::::"
