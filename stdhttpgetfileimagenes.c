...
int main(int argc, char *argv[]) {
  ...
  while(fread(lea, sizeof(char), 1, stdin)>0) {
	...
	sprintf(tipo, "jpeg");
      ...
      if(!strcmp(lee+strlen(lee)-4, ".jp2")) {
	sprintf(tipo, "jpeg");
      }
      if(!strcmp(lee+strlen(lee)-4, ".jpeg")) {
	sprintf(tipo, "jpeg");
      }
      if(!strcmp(lee+strlen(lee)-4, ".png")) {
	sprintf(tipo, "png");
      }
      if(!strcmp(lee+strlen(lee)-4, ".gif")) {
	sprintf(tipo, "gif");
      }
      if(!strcmp(lee+strlen(lee)-4, ".ico")) {
	sprintf(tipo, "ico");
      }
      if (tipo[0]!=0) {
      if(lee[0]=='/')  {
	...
	if(filo>0) {
	  ...
	  int ssize=(int)statinfo.st_size;
	  printf("HTTP/1.1 200 OK\nContent-Length: %i\nContent-Type: image/%s; \r\n\r\n",(int)statinfo.st_size , tipo);
	  free(temporal);
	  temporal=calloc(1024, 1);
	  bzero(temporal, 1024);
	  while(ssize>1024) {
	    while(fread( temporal, 1, 1024, filo)>0) fwrite(temporal, 1,1024, stdout); //printf("%c", lea[0]);
	    ssize-=1024;
	  }
	  if(fread( temporal, 1, ssize, filo)>0) fwrite(temporal, 1,ssize, stdout);
	  fclose(filo);
