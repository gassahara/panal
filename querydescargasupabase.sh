...
echo ""  | tr -d '
' > $nomprograma.lis
remotepath=$(./supabase_host)

# Polling for JSON index from Supabase Billboard Storage
curl -L "$remotepath" > $nomprograma.list.json 2>/dev/null
json=$(cat "$nomprograma.list.json")

# Parsing the Supabase JSON array using std C tools
encuentra=$(echo "$json" | $PrPWD/stdbuscaarg '"name":"')
while [ -n "$encuentra" ]; do
    filename=$(echo "$json" | $PrPWD/stdcdr '"name":"' | $PrPWD/stdcarsin '"')
    fileurl=$(echo "$json" | $PrPWD/stdcdr '"url":"' | $PrPWD/stdcarsin '"')
    
    # We filter only .js payloads
    isjs=$(echo "$filename"|$PrPWD/stdbuscaarg ".js")
    if [ -n "$isjs" ];then
        # Using comma delimiter to store url and filename
        echo "$fileurl,$filename" >> $nomprograma.lis
    fi
    
    json=$(echo "$json" | $PrPWD/stdcdr '"url":"')
    encuentra=$(echo "$json" | $PrPWD/stdbuscaarg '"name":"')
done
...
'|$PrPWD/stdcdr " ")
    ...
    # listf is "url,filename"
    fileurl=$(echo "$listf" | $PrPWD/stdcarsin ',')
    filename=$(echo "$listf" | $PrPWD/stdcdr ',')

    chacha=$(echo "$filename"|$PrPWD/chacha20|$PrPWD/stddelcar " ")
    encuentra=""
    encuentra=$(cat $nomprograma.memoria 2>/dev/null | $PrPWD/stdbuscaarg ";$filename;$chacha;")
    
    if [ -z "$encuentra" -a -n "$filename" ];then
	ttest=$(echo "$filename"|tr -d '
'|$PrPWD/stdbuscaarg ";")
	if [ -n "$ttest" ];then
	    fn=$(echo "$filename"|tr -d '
'|$PrPWD/stdcdr ";" )
	else
	    fn=$(echo "$filename"|tr -d '
' )
	fi
	tamano=$(echo "$filename"|tr -d '
'|$PrPWD/stdcarsin ";" )
	ttest=$(echo "$fn" |tr -d '
'|$PrPWD/stddelcar " ")
	echo "<< $ttest .:. fn $fn ($filename) >>"
	if [ -n "$ttest" ];then
		...
		if [ 0$opens -gt 0 -a "$balan" = "0"  ];then
		    echo ";$filename;$chacha;" >> $nomprograma.memoria
                    echo "<.<><><><><><><><   $fn"
                    sleep 3
...
' > $datosdelc.js
				    ...
				    echo "Would normally upload $datosdelc.js response directly to storage if required."
				else
				    echo "======================="
				    mkdir -p $PrPWD/users
				    mkdir -p $PrPWD/users/input
				    mkdir -p $PrPWD/users/input/unencrypted
				    cp -v $datosdelc.c $PrPWD/users/input/$usuarioo/ 2>/dev/null
