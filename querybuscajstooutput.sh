...
if [ -z "$encuentra" -a -n "$listf" ];then
	...
	if [ 0$len -gt 0 ];then
		...
		if [ 0$len -gt 0 ];then iv=$(echo -n "$iv,{");fi
		len=$(expr 0$len - 1);
	    done
	    
	    cat $outputdelc.c | $PrPWD/stdcar "char iv[1][16]=" > $outputdelc-2.c
	    echo "$iv" >> $outputdelc-2.c
	    cat $outputdelc.c | $PrPWD/stdcdr "char iv[1][16]=" | $PrPWD/stdcdr ";" >> $outputdelc-2.c
	    mv $outputdelc-2.c $outputdelc.c
