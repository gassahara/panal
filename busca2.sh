#!/bin/bash
query=$1
if [ -f "palabras2/$query" ];then
    cabeza=$(head -n1 palabras2/$query)
    numero=$(cat palabras2/$query | ./stdbuscaarg_count "$query")
    tienedireccion=0;
    esarc=0
    esdir=0
    ind=1
	...
	if [ -f "$t" ];then
	    ...
	    nnn=$(./hazgethttp "$t" 2>/dev/null | wc -c | tr -d " ")
	...
	./busca2.sh "$nombro"
	i=$((i+1))
    done < <(find palabras2/ -maxdepth 1 -iname "*$query*")
    if [ $i -lt 1 ];then
