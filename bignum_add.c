...
#define BN_ARRAY_SIZE    2 //(2 / WORD_SIZE)
...
void bignum_add(struct bn* a, struct bn* b, struct bn* c) {
  ...
  free(c->array);
  if(a->len > b->len) {
    c->array=calloc(a->len+1, sizeof(DTYPE));
    c->len=a->len+1;
  }  else {
  ...
  while(i<a->len || i < b->len) {
    tmp = carry;
    if(i < b->len ) tmp=tmp + (DTYPE_TMP)b->array[i];
    else termina++;
    if(i < a->len ) tmp=tmp + (DTYPE_TMP)a->array[i];
    else termina++;
    if(termina==2) break;
    else termina=0;
    carry = (tmp > MAX_VAL);
...
int main( int argc, char *argv[] ) {
  ...
  int ssib=64;
  int ssize=64;
  buffer=calloc(ssib, sizeof(char));
  char *temporal;
  temporal=calloc(ssib, sizeof(char));
  memset(buffer, 0, ssib);
  memset(temporal, 0, ssib);
  int iniciazero=0;
  char buff[2];
  int r=1;
  int rr=0;
  ...
  bignum_add2(&Bnum, &Anum);
  printf("%s", bignum_to_string(&Bnum));
}
